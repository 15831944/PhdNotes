...
BOOL CPreviewCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
	if (m_pView)
	{
		if (zDelta < 0)
...
void CPreviewCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	SetFocus();
	//设置光标样式
	m_bOrbiting = true;
...
void CPreviewCtrl::OnMButtonDown(UINT nFlags, CPoint point) 
{
	//开始移动
	m_bPanning = true;
	SetCapture();
...
void CPreviewCtrl::OnMButtonUp(UINT nFlags, CPoint point) 
{
	ReleaseCapture();
	m_bPanning = false;
...
//函数功能:鼠标滚轮放大缩小视图
// void CPreviewCtrl::OnMouseMove(UINT nFlags, CPoint point) 
// {
// 	if (m_pView)
// 	{
// 		if (m_bPanning)
...
LRESULT CPreviewCtrl::OnNcHitTest(CPoint point)
{
	return HTCLIENT;
}


void CPreviewCtrl::OnSetFocus(CWnd* pOldWnd) 
{
	::SetClassLong(m_hWnd, GCL_HCURSOR, (long)m_hCrossCursor);

}

void CPreviewCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bOrbiting = false;
	::SetClassLong(m_hWnd,GCL_HCURSOR,(long)m_hCrossCursor);
	ReleaseCapture();
...
void CPreviewCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	double x = point.x;
	double y = point.y;
	CString sx, sy;
...
//函数功能:传入dwg文件即可预览
BOOL CPreviewCtrl::Init(LPCTSTR szDwgFile)
{ 
	Clear();
...
BOOL CPreviewCtrl::Init(AcDbDatabase *pDb, CString strBlkTblRcd)
{ 
	Clear();
...
bool CPreviewCtrl::GetActiveViewPortInfo (ads_real &height, ads_real &width, 
...
//函数功能:初始化图形系统
void CPreviewCtrl::InitGS(HINSTANCE hRes)
{
	// 加载光标
...
BOOL CPreviewCtrl::InitInner(AcDbDatabase *pDb, CString strBlkTblRcd)
{
	if (pDb == NULL)
	{
...
void CPreviewCtrl::Mid(const AcGePoint3d& pt1, const AcGePoint3d& pt2, AcGePoint3d& ptMid)
{ 
...
//函数功能:获得块中的所有实体
void CPreviewCtrl::GetAllEnt(const AcDbObjectId& idBlockRec, AcDbObjectIdArray& IDArray)
{
...
//函数功能:获得实体的范围
Acad::ErrorStatus CPreviewCtrl::GetEntExtents(const AcDbObjectId& idEnt, AcDbExtents& extents)
{
	Acad::ErrorStatus es;
...
void CPreviewCtrl::GetEntExtents(const AcDbObjectIdArray& aridEnt, AcDbExtents& extents)
{
