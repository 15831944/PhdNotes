...
PhdConfig::PhdConfig(const CString& strFilePath)
...
void PhdConfig::SetFilePath(LPCTSTR szFilePath)
{
...
bool PhdConfig::SetValue(LPCTSTR szSection, LPCTSTR szName, LPCTSTR szValue) const
{
	return SetIniValue(szSection, szName, szValue);
}

bool PhdConfig::SetValue(LPCTSTR szSection, LPCTSTR szName, int value) const
{
	TCHAR sz[10];
	_itot_s(value, sz, 10);
...
bool PhdConfig::SetValue(LPCTSTR szSection, LPCTSTR szName, double value) const
{
	CString strValue;
...
CString PhdConfig::GetValue(LPCTSTR szSection, LPCTSTR szName, LPCTSTR szDefault /*= nullptr*/) const
{
	return GetIniValue(szSection, szName, szDefault);
}

int PhdConfig::GetValue(LPCTSTR szSection, LPCTSTR szName, int nDefault) const
{
	TCHAR sz[10];
...
double PhdConfig::GetValue(LPCTSTR szSection, LPCTSTR szName, double fDefault) const
{
	CString strDefault;
...
std::vector<CString> PhdConfig::GetAllSections() const
{
	/*
	本函数基础：
...
BOOL PhdConfig::DelSection(LPCTSTR lpSection) const
{
	return WritePrivateProfileString(lpSection, NULL, NULL, m_strFilePath);
}

BOOL PhdConfig::DelAllSections() const
{
...
BOOL PhdConfig::DelKey(LPCTSTR lpSection, LPCTSTR lpKey) const
{
	return WritePrivateProfileString(lpSection, lpKey, NULL, m_strFilePath);
}

int PhdConfig::GetSectionData(LPCTSTR lpszSection, std::vector<CString>& vecKey, std::vector<CString>& vecValue) const
{
	/*
	本函数基础：
...
CString PhdConfig::GetIniValue(LPCTSTR section, LPCTSTR valueName, LPCTSTR sz_default) const
{
	CString value;
	//先去Appdata目录下找
...
bool PhdConfig::SetIniValue(LPCTSTR section, LPCTSTR valueName, LPCTSTR value) const
{
...
CString PhdConfig::GetAppdataDir() const
{
